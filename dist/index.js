...
var ENTRIES_FILE = path.join(process.cwd(), "data", "entries.json");
...
async function startServer() {
  ...
  app.post("/api/entries", (req, res) => {
    const entries = readEntries();
    const body = req.body;
    const idx = entries.findIndex((e) => e.date === body.date);
  ...
  app.delete("/api/entries/:date", (req, res) => {
    const entries = readEntries();
    writeEntries(entries.filter((e) => e.date !== req.params.date));
    res.json({ ok: true });
  });
