...
**Learning:** When defining a global accessibility baseline in a Tailwind CSS application (e.g., `:focus-visible` states and `prefers-reduced-motion` settings in `client/src/index.css`), using `!important` is sometimes necessary.
**Action:** Use `!important` as an exception to the 'no !important' rule specifically for these global accessibility directives in `@layer base` to ensure they are not unintentionally masked or overridden by individual component or utility classes.
